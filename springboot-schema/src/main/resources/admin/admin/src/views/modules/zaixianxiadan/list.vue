<template>
	<div class="main-content" :style='{"color":"#666","padding":"0px 30px 30px","fontSize":"14px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","margin":"0 0 20px","borderRadius":"0px","textAlign":"left","background":"none","display":"block","width":"100%","fontSize":"inherit","order":"2"}' >
					...
					<div :style='{"margin":"0 0px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="状态" prop="zhuangtai">
				...
				<el-row class="actions" :style='{"margin":"0px 0 20px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit","order":"1"}'>
					<el-button class="add" v-if="isAuth('zaixianxiadan','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('zaixianxiadan','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"0px solid #eee","width":"100%","padding":"0","margin":"0px 0 0","borderRadius":"12px","background":"rgba(255,255,255,.9)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('zaixianxiadan','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('zaixianxiadan','订单分配')" @click="dingdanfenpeiCrossAddOrUpdateHandler(scope.row,'cross','','','zhuangtai','该订单已分配','已分配,未分配'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								订单分配
							</el-button>
							<el-button class="edit" v-if=" isAuth('zaixianxiadan','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('zaixianxiadan','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		<dingdanfenpei-cross-add-or-update v-if="dingdanfenpeiCrossAddOrUpdateFlag" :parent="this" ref="dingdanfenpeiCrossaddOrUpdate"></dingdanfenpei-cross-add-or-update>
...
import dingdanfenpeiCrossAddOrUpdate from "../dingdanfenpei/add-or-update";
	export default {
		data() {
			return {
				zhuangtaiOptions: [],
				...
				dingdanfenpeiCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			dingdanfenpeiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.dingdanfenpeiCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.dingdanfenpeiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.dingdanfenpeiCrossaddOrUpdate.init(row.id,type);
				});
			},
    ...
    init () {
          this.zhuangtaiOptions = "已分配,未分配".split(',')
    },
    search() {
      this.pageIndex = 1;
      this.getDataList();
	...
	zaixianxiadanstatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'zaixianxiadan/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
