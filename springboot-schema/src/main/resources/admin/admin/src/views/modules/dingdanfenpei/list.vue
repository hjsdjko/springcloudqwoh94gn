<template>
	<div class="main-content" :style='{"color":"#666","padding":"0px 30px 30px","fontSize":"14px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"margin":"0px 0 20px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit","order":"1"}'>
					<el-button class="add" v-if="isAuth('dingdanfenpei','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('dingdanfenpei','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('dingdanfenpei','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
			...
			<div :style='{"border":"0px solid #eee","width":"100%","padding":"0","margin":"0px 0 0","borderRadius":"12px","background":"rgba(255,255,255,.9)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('dingdanfenpei','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('dingdanfenpei','开始行驶')" @click="xingchengxinxiCrossAddOrUpdateHandler(scope.row,'cross','是','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								开始行驶
							</el-button>
							<el-button class="btn8" v-if="isAuth('dingdanfenpei','联系司机')" @click="lianxisijiCrossAddOrUpdateHandler(scope.row,'cross','是','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								联系司机
							</el-button>
							<el-button class="btn8" v-if="isAuth('dingdanfenpei','联系用户')" @click="lianxiyonghuCrossAddOrUpdateHandler(scope.row,'cross','是','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								联系用户
							</el-button>
							<el-button class="edit" v-if=" isAuth('dingdanfenpei','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('dingdanfenpei','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		<xingchengxinxi-cross-add-or-update v-if="xingchengxinxiCrossAddOrUpdateFlag" :parent="this" ref="xingchengxinxiCrossaddOrUpdate"></xingchengxinxi-cross-add-or-update>
		<lianxisiji-cross-add-or-update v-if="lianxisijiCrossAddOrUpdateFlag" :parent="this" ref="lianxisijiCrossaddOrUpdate"></lianxisiji-cross-add-or-update>
		<lianxiyonghu-cross-add-or-update v-if="lianxiyonghuCrossAddOrUpdateFlag" :parent="this" ref="lianxiyonghuCrossaddOrUpdate"></lianxiyonghu-cross-add-or-update>
...
import xingchengxinxiCrossAddOrUpdate from "../xingchengxinxi/add-or-update";
import lianxisijiCrossAddOrUpdate from "../lianxisiji/add-or-update";
import lianxiyonghuCrossAddOrUpdate from "../lianxiyonghu/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				xingchengxinxiCrossAddOrUpdateFlag: false,
				lianxisijiCrossAddOrUpdateFlag: false,
				lianxiyonghuCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			xingchengxinxiCrossAddOrUpdate,
			lianxisijiCrossAddOrUpdate,
			lianxiyonghuCrossAddOrUpdate,
		},
		methods: {
			contentStyleChange() {
			...
			xingchengxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.xingchengxinxiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','dingdanfenpei');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.xingchengxinxiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.xingchengxinxiCrossaddOrUpdate.init(row.id,type);
				});
			},
			lianxisijiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				if(crossOptAudit=='是'&&row.sfsh!='是') {
					this.$message({
						message: "请审核通过后再操作",
						type: "success",
				...
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.lianxisijiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','dingdanfenpei');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.lianxisijiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.lianxisijiCrossaddOrUpdate.init(row.id,type);
				});
			},
			lianxiyonghuCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				if(crossOptAudit=='是'&&row.sfsh!='是') {
					this.$message({
						message: "请审核通过后再操作",
						type: "success",
				...
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.lianxiyonghuCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','dingdanfenpei');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.lianxiyonghuCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.lianxiyonghuCrossaddOrUpdate.init(row.id,type);
				});
			},
		...
		this.$refs["shBatchForm"].validate(valid => {
			if(valid){
				this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
					...
					this.$http({
						url: "dingdanfenpei/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
						method: "post",
						data: this.batchIds
					}).then(({ data }) => {
						if (data && data.code === 0) {
	...
	dingdanfenpeistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'dingdanfenpei/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
