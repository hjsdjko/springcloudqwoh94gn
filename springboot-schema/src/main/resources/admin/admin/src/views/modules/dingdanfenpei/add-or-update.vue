	...
	<div class="addEdit-block" :style='{"padding":"0px 0px 30px","fontSize":"14px","color":"#000","background":"none"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="date" v-if="type!='info'" label="分配时间" prop="fenpeishijian">
					<el-date-picker
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-else-if="ruleForm.fenpeishijian" label="分配时间" prop="fenpeishijian">
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="select" v-if="type!='info'" label="司机账号" prop="sijizhanghao">
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-else-if="ruleForm.sijizhanghao" label="司机账号" prop="sijizhanghao">
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="select" v-if="type!='info'" label="路线名称" prop="luxianmingcheng">
					<el-select :disabled="ro.luxianmingcheng" @change="luxianmingchengChange" v-model="ruleForm.luxianmingcheng" placeholder="请选择路线名称">
						<el-option
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-else-if="ruleForm.luxianmingcheng" label="路线名称" prop="luxianmingcheng">
...
export default {
	...
	methods: {
		...
		init(id,type) {
            ...
            this.$http({
				url: `option/luxianguihua/luxianmingcheng`,
				method: "get"
            }).then(({ data }) => {
				if (data && data.code === 0) {
			...
			// 下二随
			luxianmingchengChange () {
				this.$http({
					url: `follow/luxianguihua/luxianmingcheng?columnValue=`+ this.ruleForm.luxianmingcheng,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
						if(data.data.sijixingming){
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `dingdanfenpei/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.dingdanfenpeiCrossAddOrUpdateFlag = false;
											this.parent.search();
