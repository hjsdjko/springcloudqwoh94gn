<template>
	<div class="addEdit-block" :style='{"padding":"0px 0px 30px","fontSize":"14px","color":"#000","background":"none"}'>
		...
		>
			<template >
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-if="type!='info'"  label="联系标题" prop="lianxibiaoti">
					<el-input v-model="ruleForm.lianxibiaoti" placeholder="联系标题" clearable  :readonly="ro.lianxibiaoti"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="联系标题" prop="lianxibiaoti">
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="date" v-if="type!='info'" label="联系时间" prop="lianxishijian">
					<el-date-picker
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-else-if="ruleForm.lianxishijian" label="联系时间" prop="lianxishijian">
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-if="type!='info'"  label="联系内容" prop="lianxineirong">
					<editor 
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else-if="ruleForm.lianxineirong" label="联系内容" prop="lianxineirong">
                    <span :style='{"padding":"0 20px","color":"inherit","background":"#fff","display":"inline-block","width":"80%","fontSize":"14px","lineHeight":"40px","fontWeight":"500"}' v-html="ruleForm.lianxineirong"></span>
                </el-form-item>
...
export default {
	data() {
		...
		return {
			...
			rules: {
				lianxibiaoti: [
					{ required: true, message: '联系标题不能为空', trigger: 'blur' },
				],
				zhanghao: [
				],
				xingming: [
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.lianxineirong = this.ruleForm.lianxineirong.replace(reg,'../../../springcloudqwoh94gn/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `lianxiyonghu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.lianxiyonghuCrossAddOrUpdateFlag = false;
											this.parent.search();
